namespace NServiceBus
{
    ...
    class RouteToGatewayBehavior : Behavior<IRoutingContext>
    {
        ...
        {
            RouteThroughGateway _;
            if (context.Extensions.TryGet(out _))
            {
                string siteKeyHeader;
                if (context.Message.Headers.TryGetValue(Headers.DestinationSites, out siteKeyHeader))
                {
